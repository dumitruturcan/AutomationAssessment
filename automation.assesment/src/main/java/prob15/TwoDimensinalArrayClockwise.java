...
public class TwoDimensinalArrayClockwise {
	...
	public ArrayList<ArrayList<Integer>> rotateClokwise2DArrayList(ArrayList<ArrayList<Integer>> array,
		...
		case 3: {
			...
			line = array.get(array.size() - 1).size() - 1;
			row = 0;
			array = threeRotations(line, row, array);
			break;
		}

		}

		return deleteNullValues(array);
	}

	public ArrayList<ArrayList<Integer>> deleteNullValues(ArrayList<ArrayList<Integer>> array) {
	...
	private ArrayList<ArrayList<Integer>> twoRotations(int line, int row, ArrayList<ArrayList<Integer>> array) {
		...
		for (int i = array.size() - 1; i >= 0; i--) {
			...
			for (int j = array.size() - 1; j >= 0; j--)
				...
				result.get(k).add(array.get(i).get(j));

		}
		return result;
	}

	private ArrayList<ArrayList<Integer>> threeRotations(int line, int row, ArrayList<ArrayList<Integer>> array) {
	...
	protected ArrayList<ArrayList<Integer>> convertStringListTo2DArray(ArrayList<String> inputLines) {
